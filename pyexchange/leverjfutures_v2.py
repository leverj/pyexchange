...
class LeverjFuturesAPI(PyexAPI):
    ...
    def createNewOrder(self, side: str, price: str, triggerPrice: str, quantity: str, orderInstrument: dict, orderType: str, leverage: float = 0.0, reduceOnly: bool = False) -> dict:
        price_precision = orderInstrument.get('quoteSignificantDigits')
        quantity_precision = orderInstrument.get('baseSignificantDigits')
        # default leverage is set to 0.0 which means you aren't using any leverage. If you want 5K DAI position to control 10K DAI worth of BTC, use leverage of 2
        order = {
